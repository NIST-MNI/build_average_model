...
begin
  ...
  stagger=false #make delays at the beginning of each massive parallel job
   # List of arguments.
    ...
    opts.banner = "Usage: #{me} [Options] <file_list>"
    opts.separator ""
...
#     end
    ...
    opts.on('--dd-iterations <iterations>','number of iterations per DD step,default 20',Numeric) do |o|
      ...
      puts "DD_iterations=#{dd_iterations}"
    end
  ...
  write_list(masks,mask_avg_list) if !in_masks.empty? 
  ...
  (0 .. fit_steps.size()-1).each do |f|
    ...
    (1..iterations).each do |iti|
          ...
          batch.submit(step1_name, prev_step,logfile) do |e|
            ...
            unless disable_linear && non_linear_on
              ...
              if symmetric #TODO implement symmetric part
                ...
                e<<[bin_dir+'do_sym_nonlinear_registration_dd', file,fliped, cur_model, 
                    file_mask,fliped_mask, 
                    cur_model_mask,step,prev_xfm,
                    output,output_i,output_i+'.flip.xfm',dd_iterations, dd_options]
                    
                cleanup_list << output_i+'.flip.xfm'
                cleanup_list << output_i+'.flip_grid_0.mnc'
                xfm_i_list << output_i+'.flip.xfm'
              else  
                e << [bin_dir+'do_nonlinear_registration_dd', file, 
                      cur_model, file_mask, cur_model_mask, 
                      step, prev_xfm, output, output_i,dd_iterations, dd_options]
              end
      ...
      (0 .. (in_files.length()-1)).each do |ff|
        ...
        unless File.exist?(output_minc) && File.exist?(file_mask) && (File.exists?(output_flip_minc) ||  !symmetric )
          batch.submit(step3_name, step2_name,logfile) do |e|
            ...
            resample_script='do_concat_resample_mod'
            if spline 
              resample_script='do_concat_resample_itk_mod'
            end
            if symmetric
              e<<[bin_dir+resample_script,input,file,xfm_avg,
                 cur_model,file_mask, output,output_minc,output_mask,output_flip_minc,step]
            else
              e<<[bin_dir+resample_script,input,file,xfm_avg,
                 cur_model,file_mask, output,output_minc,output_mask,'none',step]
            end
          end
        end
      ...
      write_list(masks,mask_avg_list) 
